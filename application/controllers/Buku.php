...
class Buku extends CI_Controller
{
	...
	public function hapusBuku()
	{
		...
		$namaBuku = $this->ModelBuku->bukuWhere($where)->row_array();
		$this->ModelBuku->hapusBuku($where);

		$this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert"> Buku berjudul '.$namaBuku['nama_kategori']. ' berhasil dihapus</div>');
		redirect('buku');
	}
	...
	public function ubahBuku()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$old_image = $this->ModelBuku->bukuWhere(['id' => $this->input->post('id')])->row_array();
			...
			if($upload_gambar){
				...
				if ($this->upload->do_upload('image')) {
					if($old_img != 'default.jpg'){
						unlink(FCPATH . 'asset/img/upload/'. $old_image['image']);
					}

					$new_img = $this->upload->data('file_name');
					$dataConfig['image'] = $new_img;
				}else{
					$this->session->set_flashdata('pesan', '<div
					class="alert alert-danger alert-message" role="alert">'.$this->upload->display_errors().'</div>');
					redirect('buku/ubahBuku/'. $this->input->post('id'));
				}
			}			

			$this->ModelBuku->updateBuku($dataConfig, ['id' => $this->input->post('id')]);
			redirect('buku');
		}
	}
	...
	public function hapusKategori()
	{
		...
		$namaKategori = $this->ModelBuku->KategoriWhere($where)->row_array();
		$this->ModelBuku->hapusKategori($where);

		$this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert"> Kategori '.$namaKategori['nama_kategori']. ' berhasil dihapus</div>');
		redirect('buku/kategori');
	}
}